...
function render_frontend_head($title = '', $meta_description = '', $active_page = '')
{
    ...
    <meta property="og:site_name"
        content="<?php echo htmlspecialchars($seo_settings['site_title'] ?? 'Frankfurt Innenausbau'); ?>">
    ...
    <!-- Fonts -->
    <link
        href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;600&family=Roboto:wght@300;400;500&display=swap"
        rel="stylesheet">
    ...
    <?php if (!empty($seo_settings['facebook_pixel'])): ?>
        ...
        <noscript><img height="1" width="1" style="display:none"
                src="https://www.facebook.com/tr?id=<?php echo htmlspecialchars($seo_settings['facebook_pixel']); ?>&ev=PageView&noscript=1" /></noscript>
    <?php endif; ?>
...
function render_frontend_layout($options = [])
{
    ...
    <body class="<?php echo $opts['body_class']; ?>">
        ...
        <!-- Progress Bar -->
        <div id="progress-bar" class="fixed top-0 left-0 w-0 h-1 bg-gray-500 z-[9999] transition-all duration-150 ease-out">
        </div>
...
function render_frontend_navigation($active_page = '')
{
    ...
    $menu_items = [
        'home' => ['url' => './', 'title' => $is_german ? 'Startseite' : 'Главная', 'anchor' => '#hero'],
    ...
    <nav class="bg-white shadow-sm fixed w-full z-50">
            ...
            <div class="flex justify-around md:justify-between items-center h-16">
                ...
                <div class="hidden lg:flex space-x-6 xl:space-x-8 nav-menu-container">
                    <?php foreach ($menu_items as $page => $item): ?>
                        ...
                        <a href="<?php echo $item['url']; ?>"
                            class="nav-menu-item <?php echo $is_active ? 'text-accent-blue font-medium' : 'text-text-secondary hover:text-accent-blue'; ?> transition-colors whitespace-nowrap">
                ...
                <div class="nav-controls flex items-center space-x-3 ml-1">
                    ...
                    <button id="mobile-menu-button"
                        class="lg:hidden p-2 rounded-md text-text-secondary hover:text-accent-blue focus:outline-none">
        ...
        <div id="mobile-menu" class="lg:hidden hidden bg-white border-t border-gray-200 shadow-lg">
            <div class="px-4 py-2 space-y-1">
                <?php foreach ($menu_items as $page => $item): ?>
                    <a href="<?php echo $item['url']; ?>"
                        class="block px-3 py-2 text-text-secondary hover:text-accent-blue hover:bg-gray-50 rounded-md transition-colors mobile-menu-link">
                        <?php echo htmlspecialchars($item['title']); ?>
                    </a>
                <?php endforeach; ?>
                <div class="pt-2 pb-1">
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="flex space-x-4">
                        <?php if (!empty($contact_info['social']['facebook'])): ?>
                            <a href="<?php echo htmlspecialchars($contact_info['social']['facebook']); ?>" target="_blank"
                                class="bg-accent-blue text-white p-3 rounded-full hover:bg-opacity-80 transition-colors">
                        ...
                        <?php if (!empty($contact_info['social']['instagram'])): ?>
                            <a href="<?php echo htmlspecialchars($contact_info['social']['instagram']); ?>" target="_blank"
                                class="bg-accent-blue text-white p-3 rounded-full hover:bg-opacity-80 transition-colors">
                                <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                        ...
                        <?php if (!empty($contact_info['social']['linkedin'])): ?>
                            <a href="<?php echo htmlspecialchars($contact_info['social']['linkedin']); ?>" target="_blank"
                                class="bg-accent-blue text-white p-3 rounded-full hover:bg-opacity-80 transition-colors">
                                <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                        ...
                        <?php if (!empty($contact_info['social']['whatsapp'])): ?>
                            <a href="https://wa.me/<?php echo preg_replace('/[^0-9]/', '', $contact_info['social']['whatsapp']); ?>"
                                target="_blank"
                                class="bg-accent-blue text-white p-3 rounded-full hover:bg-opacity-80 transition-colors">
                                <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                        ...
                        <?php if (!empty($contact_info['social']['telegram'])): ?>
                            <a href="https://t.me/<?php echo ltrim($contact_info['social']['telegram'], '@'); ?>"
                                target="_blank"
                                class="bg-accent-blue text-white p-3 rounded-full hover:bg-opacity-80 transition-colors">
                                <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
            ...
            <div class="border-t border-gray-700 mt-12 pt-8">
                <div class="flex flex-col md:flex-row justify-between items-center">
                    <p class="text-gray-400 text-sm">
                        © 2024 Frankfurt Innenausbau.
                        <?php echo $is_german ? 'Alle Rechte vorbehalten.' : 'Все права защищены.'; ?>
                    </p>
                    <div class="flex space-x-6 mt-4 md:mt-0">
                        <a href="#"
                            class="text-gray-400 hover:text-white text-sm transition-colors"><?php echo $is_german ? 'Datenschutz' : 'Политика конфиденциальности'; ?></a>
                        <a href="#"
                            class="text-gray-400 hover:text-white text-sm transition-colors"><?php echo $is_german ? 'Nutzungsbedingungen' : 'Условия использования'; ?></a>
                        <a href="admin/login"
                            class="text-gray-400 hover:text-white text-sm transition-colors"><?php echo $is_german ? 'Anmelden' : 'Вход'; ?></a>
                    </div>
                </div>
            </div>
        </div>
...
function render_frontend_scripts()
{
    ...
    <script>
        ...
        function switchLanguage(targetLanguage) {
            ...
            const isGerman = currentUrl.indexOf('/de/') !== -1;
